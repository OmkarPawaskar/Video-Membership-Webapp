...
from app.users.exceptions import InvalidEmailException, UserHasAccountException
...
class User(Model):
    ...
    def create_user(email, password=None):
        ...
        if q.count() != 0:
            raise UserHasAccountException()
        msg, valid, email = validators._validate_email(email)
        if not valid:
