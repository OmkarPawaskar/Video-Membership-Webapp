...
@login_required
def playlist_create_view(request: Request, title:str = Form(...)): #To declare a field as required, you may declare it using just an annotation, or you may use an ellipsis (...) as the value
    raw_data = {
        "title" : title,
...
def playlist_detail_view(request: Request, db_id:uuid.UUID):
    q = get_obj_or_404(Playlist, db_id=db_id)
...
def playlist_video_add_view(
    ...
    return render(request, "playlists/htmx/add_video.html", context)
...
def playlist_video_add_post_view(
    ...
    is_htmx=Depends(is_htmx),
    url:str = Form(...), 
    title:str = Form(...),): #To declare a field as required, you may declare it using just an annotation, or you may use an ellipsis (...) as the value
    raw_data = {
        "title" : title,
        "url" : url,
        "user_id" : request.user.username,
...
def playlist_remove_video_item_view(
    ...
    if not is_htmx:
        raise HTTPException(status_code=400)
    try:
        obj = get_obj_or_404(Playlist, db_id=db_id)
    except:
        return HTMLResponse("Error. Please reload the page.")
    if not request.user.is_authenticated:
        return HTMLResponse("Please login to continue.")
