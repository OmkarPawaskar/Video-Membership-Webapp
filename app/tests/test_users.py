...
def test_duplicate_user(setup):
    with pytest.raises(Exception):
        User.create_user(email="test@test.com", password = "abcde1234")

def test_invalid_email(setup):
    with pytest.raises(Exception):
        User.create_user(email="test@test", password = "abcde1234")
...
def test_valid_password(setup):
    ...
    assert user_obj.verify_password("abcde1234") == False